...
fn bin_path() -> PathBuf {
    env::var("CARGO_BIN_EXE_linux-x11-sandbox")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("target/debug/linux-x11-sandbox"))
}
...
impl DaemonGuard {
    async fn new() -> Self {
        ...
        let name = format!("lxs-test-{}.sock", std::process::id());
        let socket = tmp.join(&name);
        let pid = tmp.join(format!("lxs-test-{}.pid", std::process::id()));
        let _ = tokio::fs::remove_file(&socket).await;
        let _ = tokio::fs::remove_file(&pid).await;

        let mut child = Command::new(bin_path())
            .arg("serve")
            .env("LXS_SOCKET_PATH", &socket)
            .env("LXS_PID_PATH", &pid)
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
            .expect("spawn daemon");
...
#[tokio::test]
async fn create_and_destroy_sandbox_display() {
    let daemon = DaemonGuard::new().await;
    let mut conn = daemon.connect().await;
    let create = conn.call_tool("lxs_display_create", json!({})).await;
    ...
    let info = conn
        .call_tool("lxs_display_info", json!({"display_id": display_id}))
        .await;
    assert!(
    ...
    let destroy = conn
        .call_tool("lxs_display_destroy", json!({"display_id": display_id}))
        .await;
...
async fn launch_app_and_take_screenshot() {
    ...
    let shot = conn
        .call_tool("lxs_capture_screenshot", json!({"display_id": display_id}))
        .await;
